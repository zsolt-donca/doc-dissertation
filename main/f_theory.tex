...
\end{lstlisting}
This defines the abstract method \texttt{name}. The method is implicitly abstract (without having to use the keyword \texttt{abstract}) as no method body follows the declaration. This trait can now be mixed in into a class:
\begin{lstlisting}
class Cat(nickname: String) extends Named {
  override def name: String = nickname
}
\end{lstlisting}
Just as with ordinary inheritance, any \texttt{Cat} instance can be treated as an instance of the trait \texttt{Named}:
...
\noindent A class can mix in any number of traits, and can even have a subclass:
\begin{lstlisting}
class Animal { ... }
trait FourLegged { ... }
trait Mammal { ... }
...
\subsubsection{Thin versus rich interface}
...
One major use of traits is to automatically add methods to a class in terms of methods the class already has. This way, traits can enrich a \emph{thin} interface making it into a \emph{rich} interface. 
...
\end{lstlisting}
Here, the trait \texttt{TwoNames} extends the trait \texttt{Named}, implementing the method \texttt{name} with the full name, while declaring the methods \texttt{firstName} and \texttt{lastName} abstract. It also defines the convenience method \texttt{lexical\-Full\-Name}. To implement a trait like this in Java prior to Java 8, one could use an abstract class in Java, eliminating the possibility of using other base classes in extending classes (maybe of similar design). Another possibility would be to make \texttt{TwoNames} an interface, to the move the implementation of the methods \texttt{fullName}, \texttt{lexicalFullName} and \texttt{name} to a utility class, and to reference that utility class in all classes extending the interface. While this workaround is technically possible, it is cumbersome and rarely preferred.



\subsubsection{Linearization}

Traits are a way to inherit from multiple class-like constructs, but they differ in important ways from the multiple inheritance present in many languages. One difference is especially important: the interpretation of \emph{super}. With multiple inheritance, the method called by a super call can be determined right where the call appears. With traits, the method called is determined by a linearization of the classes and traits that are mixed into a class, enabling a kind of stacked modifications similar to the \emph{decorator} pattern.
...
// prints: I am a cat, a domesticated feline, that is, a carnivorous furry four-legged animal
\end{lstlisting}

The linearization goes from right to left, and from the bottom to the top of the hierarchy:\[ \texttt{Cat} \rightarrow \texttt{Domesticated} \rightarrow \texttt{Feline} \rightarrow \texttt{Carnivorous} \rightarrow \texttt{Furry} \rightarrow \texttt{FourLegged} \rightarrow \texttt{Animal} \]

In Scala, classes and traits are always are always inherited only once, meaning that there won't be four different instances of the class \texttt{Animal} in a \texttt{Cat} instance \footnote{contrary to the (non-virtual) multiple inheritance of C++, one of the most widely used languages supporting multiple inheritance}. Practically, the \texttt{extends Animal} part in the above traits make the class \texttt{Animal} a requirement of the traits, and the class \texttt{Cat} automatically extends the class \texttt{Animal} through the traits. Mixing in the trait \texttt{Feline} to a class already having a different base class results in compilation error:
\begin{lstlisting}
class Plant
class PottedPlant extends Plant with Feline
// Error: illegal inheritance; superclass Plant is not a subclass of the superclass Animal of the mixin trait Feline
\end{lstlisting}
...
\section{Scala Continuations Plugin}\label{sec:theory_scala-cps-plugin}
...
\\
\end{tabular}
\caption{Left is a delimited continuation, and right is the equivalent manually-transformed continuation passing style versions.}
...
Object-oriented reactive applications traditionally implement reactivity by using the Observer pattern  \cite{EPFL-REPORT-176887}\cite{rescala:2014}. The observer pattern relies on the concept of \emph{inversion of control} to decouple the observers from the subjects (observables), reversing the intuitive flow of the application and making the code harder to understand, analyze and maintain. Typical implementations often require a lot of boilerplate code, further complicating the code of the program. Separation of concern is often hard to achieve because the observer logic typically needs to be mixed with the application's business logic. It is difficult to maintain data consistency because the observer pattern does not incorporate the concept of transactionality or atomicity when notifying observers of changes. Also, the observer pattern enforces mutability and relying on side-effects, enforcing exclusively imperative an programming style. The paper \cite{EPFL-REPORT-176887} argues how Scala.React resolve these and many other problems with the observer pattern.
...
One of the most important problem with having to deal with observer-based architecture is the \emph{inversion of control}. When an operation consisting of multiple events needs to be modeled, the typical solution is to model it as a \emph{state machine}. 
...
Model-View-Controller is an architectural pattern for implementing user interfaces\cite{krasner_description_1988}. It divides an application into three connected parts:
\begin{itemize}
